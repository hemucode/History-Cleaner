try{
    ...
    makeArray: function (arr, results) {
      ...
      if (arr != null) {
        if (isArrayLike(Object(arr))) {
          j.merge(ret, typeof arr === "string" ? [arr] : arr);
        } else {
          push.call(ret, arr);
        }
      }
  ...
  chrome.storage.sync.get('clearhistoryinterval', function (items) {
    time = items.clearhistoryinterval;
    if (time) {
      clearTime = time;
      console.log('clearTime is retrieved from sync :' + clearTime);
      if (clearTime > 0)
        startTimer();
    }
  });
  ...
  function setClearInterval() {
    chrome.storage.sync.set({
      clearhistoryinterval: clearTime
    }, function () {
      console.log('clearTime is stored in sync : ' + clearTime);
    });
  }
  ...
  function startTimer() {
    console.log("timer started for interval : " + clearTime);
  ...
  chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    if (request.clearTime !== null && request.clearTime !== undefined) {
      ...
      setClearInterval();
    } else if (request.getTime !== null && request.getTime !== undefined) {
      if (clearTime !== null && clearTime !== undefined) {
        console.log("get Time request came from popup js");
        chrome.runtime.sendMessage({
          clearTime: clearTime
        }, function () {
          console.log('clearTime sent to popup js : ' + clearTime);
        });
      }
    }
  });
